...
class OpenAIDocumentTransformer(DocumentTransformer):
        ...
        try:
            if content_token_size > self.config.openai_token_limit:
                raise TooManyTokensException(
                    content_token_size, self.config.openai_token_limit
                )
    ...
    def executeOpenAICall(self, document: Document) -> Document:
        try:
            ...
            completion = self.config.openai.chat.completions.create(
                **function_call.dict()
            )
            ...
            try:
                ...
                raise Exception(
                    "OpenAI returned malformatted JSON"
            ...
            if len(arguments) > 1 or not isinstance(first_value, str):
                ...
                document.extracted_properties = (
                    document.extracted_properties or arguments
                )
            else:
...
class DocumentExtractor(OpenAIDocumentTransformer):
    ...
    def __init__(
        self, *, config: DoctranConfig, properties: List[ExtractProperty]
    ) -> None:
        super().__init__(config)
...
class DocumentSummarizer(OpenAIDocumentTransformer):
    ...
    def __init__(self, *, config: DoctranConfig, token_limit: int) -> None:
        ...
        self.function_description = (
            f"Summarize a document in under {self.token_limit} tokens."
        )
...
class DocumentRedactor(DocumentTransformer):
    ...
    def __init__(
        ...
        *,
        config: DoctranConfig,
        entities: List[Union[RecognizerEntity, str]] = None,
    ...
    def transform(self, document: Document) -> Document:
            ...
            if not self.interactive:
                ...
                while True:
                    ...
                    if response.lower() in ["n", "no"]:
                        raise Exception(
                            f"Cannot run presidio-anonymizer without {self.spacy_model} model."
                        )
        ...
        results = analyzer.analyze(
            text=text, entities=self.entities if self.entities else None, language="en"
        )
...
class DocumentRefiner(OpenAIDocumentTransformer):
    ...
    def __init__(self, *, config: DoctranConfig, topics: List[str] = None) -> None:
        ...
        if topics:
            ...
            self.function_description = (
                "Remove all irrelevant information from a document."
            )
        ...
        try:
            ...
            replacements = [
                ...
                for replacement in sorted(
                    document.extracted_properties["replacements"],
                    key=lambda r: r["index"],
                )
            ]
            document.transformed_content = re.sub(
                self.template_regex, lambda _: replacements.pop(0), text
            )
            return document
        except Exception as e:
            print(e)
